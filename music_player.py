...
def songPos():
    ...
    while True:
        ...
        if player.get_state() == vlc.State.Ended:
            break
        savePosition()#uff jtz wrds oft gemacht, also so jede sekunde einmal
...
def namePatternHelp():#einf eine help message sache machen.
    ...
    #system
def savePosition():
    global main_Window_Postion, plW_Position
    position_main_window = "+" + str(main_window.winfo_x()) + "+" + str(main_window.winfo_y())
    position_plW = "+" + str(plW.winfo_x()) + "+" + str(plW.winfo_y())
    if position_main_window == main_Window_Postion and position_plW == plW_Position:
        return
    main_Window_Postion = position_main_window
    plW_Position = position_plW
    filepath_settings = os.path.join(dirname,"texts/settings.txt")
    with open(filepath_settings,'r') as file:
        lines = file.readlines()
    lines[14] = "+" + str(main_window.winfo_x()) + "+" + str(main_window.winfo_y()) + "\n"
    lines[15] = "+" + str(plW.winfo_x()) + "+" + str(plW.winfo_y())
...
def loadLanguage(language,languageList):
    ...
    settingsWE_resetRelativePlWpositionButton_text_langtext = lines[235]
    return
